...
sudo apt update
sudo apt install php8.1.7 -y

# 1. instalar depend para php
sudo apt install apache2 mysql-server php8.1.7-mysql libapache2-mod-php8.1.7 php8.1.7-gd php8.1.7-mbstring php8.1.7-xml php8.1.7-curl php8.1.7-zip unzip curl -y
...
#-----------------[Extra]-------------------
# To run a script in the background in Ubuntu, you can use the ampersand sign (&) at the end

# instalar adminer "Es como phpmyadmin ya que no es compatible con PHP8>"
yes | sudo sh adminder-install.sh &

# instalar code-server "Es VScode pero desde pagWeb"
yes | sudo sh code-server-install.sh &
#-------------------------------------------


# 3. Verifica la versión de Composer:
# NOTA: me dice "Composer version 2.5.5" VS "PHP 8.1.7" -> Laravel 10.17.1?
php --version
composer --version

# 4. Crea una nueva aplicación de Laravel:
# NOTA: ningun error por ahora..
composer create-project --prefer-dist laravel/laravel myapp "10.17.1"

# copiar script para Start en el folder del proj
cp ./laravel-start.sh ./myapp/
# Hacer el script executable. Solo usar ./NombreScript.sh para executarlo
sudo chmod ug+rwx ./myapp/laravel-start.sh
...
# CREATE USER 'admin'@'localhost' IDENTIFIED WITH mysql_native_password BY 'Admin1234';
...
echo "--> comando: sudo mysql -u root < laravel-mysql.sql"
yes | sudo mysql -u root < laravel-mysql.sql
#yes | sudo mysql -u root -p '' < laravel-mysql.sql

# entrar en el folder de la app para poder executar los otros comandos
cd myapp

# ver la version de laravel del project "Tienes que estar dentro de la carpeta del project"
echo "--> Laravel proj version"
php artisan --version
...
# ========================== [ phpmyadmin ] ==============================
# IMPORTANTE : phpmyadmin NO es compatible con php8> solo con versiones de PHP7 "Por el momento".
# IMPORTANTE : Laravel 10.17.1 Solo es compatible con PHP8.1 o superior.

#sudo apt-get update -y
#sudo apt-get install -y phpmyadmin php8.1.7-mbstring
